...
func Puppeteer(cfg *config.Config, noSync bool, ctx context.Context, cancel context.CancelFunc) error {
	defer cancel()
	...
	for _, svc := range cfg.Services {
		...
		go func(svc config.Service) {
			...
			cmd, err := StartSvc(ctx, svc, noSync)
			if err != nil {
				errChan <- err
				wg.Done()
				return
			...
			RunSvc(&wg, ctx, cfg, cmd, svc, errChan, cancel)
